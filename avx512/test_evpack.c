...
static uint16_t sat_u32_to_u16(uint32_t v) { return v > 65535 ? 65535 : (uint16_t)v; }
...
int main(void) {
    ...
    for (int lane = 0; lane < 4; lane++) {
        for (int j = 0; j < 4; j++)
            TEST_ASSERT(dst.u16[lane*8+j] == sat_u32_to_u16(a.u32[lane*4+j]),
                "VPACKUSDW a lane%d j%d: %u", lane, j, dst.u16[lane*8+j]);
        for (int j = 0; j < 4; j++)
            TEST_ASSERT(dst.u16[lane*8+4+j] == sat_u32_to_u16(b.u32[lane*4+j]),
