...
static void assert_vpackusdw_result(const zmm_t *dst, const zmm_t *src1,
                                    const zmm_t *src2, const char *alias) {
    for (int lane = 0; lane < 4; lane++) {
        for (int i = 0; i < 4; i++) {
            TEST_ASSERT(dst->u16[lane * 8 + i] == sat_i32_to_u16(src1->i32[lane * 4 + i]),
                "VPACKUSDW %s src1 lane%d element%d", alias, lane, i);
            TEST_ASSERT(dst->u16[lane * 8 + 4 + i] == sat_i32_to_u16(src2->i32[lane * 4 + i]),
                "VPACKUSDW %s src2 lane%d element%d", alias, lane, i);
        }
    }
}

static void test_vpackusdw_register_aliases(void) {
    zmm_t a, b, dst;

    for (int i = 0; i < 16; i++) {
        a.i32[i] = i * 10000 - 30000;
        b.i32[i] = i * 9000 + 1000;
    }
    ...
    __asm__ volatile (
        "vmovdqu32 %1, %%zmm0\n\t"
        "vmovdqu32 %2, %%zmm1\n\t"
        "vpackusdw %%zmm1, %%zmm0, %%zmm0\n\t"
        "vmovdqu16 %%zmm0, %0"
        : "=m"(dst) : "m"(a), "m"(b) : "zmm0", "zmm1"
    );
    assert_vpackusdw_result(&dst, &a, &b, "dst=src1");

    /* dst=src2 */
    __asm__ volatile (
        "vmovdqu32 %1, %%zmm0\n\t"
        "vmovdqu32 %2, %%zmm1\n\t"
        "vpackusdw %%zmm1, %%zmm0, %%zmm1\n\t"
        "vmovdqu16 %%zmm1, %0"
        : "=m"(dst) : "m"(a), "m"(b) : "zmm0", "zmm1"
    );
    assert_vpackusdw_result(&dst, &a, &b, "dst=src2");
    ...
    assert_vpackusdw_result(&dst, &a, &a, "src1=src2");

    /* dst=src1=src2 */
    __asm__ volatile (
        "vmovdqu32 %1, %%zmm0\n\t"
        "vpackusdw %%zmm0, %%zmm0, %%zmm0\n\t"
        "vmovdqu16 %%zmm0, %0"
        : "=m"(dst) : "m"(a) : "zmm0"
    );
    assert_vpackusdw_result(&dst, &a, &a, "dst=src1=src2");
}
...
int main(void) {
    ...
    test_vpackusdw_register_aliases();
